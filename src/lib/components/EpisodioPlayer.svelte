...
<div class="card mt-4 flex min-h-[220px] flex-col items-center justify-center gap-3 p-5 text-center">
  ...
  {#if fase === 'pausa' && step?.tipo === 'responde'}
    ...
    <p class="text-2xl font-extrabold text-terracota">🎤 现在开口说！</p>
    {#key index}
  ...
  {:else if fase === 'aguardando' && step?.tipo === 'responde'}
    ...
    {:else}
      <p class="text-sm text-carvao/50">说完了？听听母语者，或直接继续。</p>
    {/if}
    {#if recUrl}
      <button class="btn bg-oceano text-white text-sm" onclick={ouvirVoce}>▶ 听自己的录音</button>
    {/if}
  {:else if fase === 'pausa' && step?.tipo === 'shadow'}
    <p class="text-2xl font-extrabold text-terracota">🗣️ 跟他们一起说！</p>
    <p class="text-sm text-carvao/50">试着不看文字说</p>
  {:else if ehPt}
...
<div class="mt-4 flex items-center justify-center gap-3">
  ...
  {#if fase === 'aguardando'}
    ...
    <button class="btn bg-salvia px-6 py-3 text-white" onclick={proximo}>⏭ 下一个</button>
  {:else}
    <button class="btn-primary px-8 py-3 text-lg" onclick={botaoCentral}>
      {#if fase === 'parado'}{index === 0 ? '▶ 开始' : '▶ 继续'}
      {:else if fase === 'fim'}↺ 再来一次
      {:else}⏸ 暂停{/if}
    </button>
  {/if}
...
{#if micErro}
  <p role="status" class="mt-1 text-center text-xs text-terracota">
    ⚠️ 无法访问麦克风——请在 iPhone 设置 › Dimmi! › 麦克风 中开启。
  </p>
{/if}
...
{#if modo === 'carro'}
  <p class="mt-3 text-center text-xs text-carvao/45">
    🚗 车载模式：自动连续播放。用方向盘按键或锁屏来播放、暂停和跳过。
  </p>
{/if}
